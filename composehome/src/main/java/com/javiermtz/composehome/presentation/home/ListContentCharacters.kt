...
import com.javiermtz.composehome.utils.Utils.DETAILS_CHARACTER_KEY

@ExperimentalCoilApi
@Composable
fun ListContentCharacters(
  characters: List<CharacterDTO>,
  navController: NavHostController
) {
  LazyRow() {
    items(characters) { character ->
      CharacterItem(characterDTO = character) { comicData ->
        navController.currentBackStackEntry?.savedStateHandle?.set<CharacterDTO>(
          DETAILS_CHARACTER_KEY, comicData
        )
...
@ExperimentalCoilApi
@Composable
fun CharacterItem(characterDTO: CharacterDTO, characterListener: (CharacterDTO) -> Unit) {
  Card(
    ...
    modifier = Modifier
      ...
      .height(ITEM_HEIGHT)
      .clickable {
        characterListener(characterDTO)
      }
  ) {
    Column(
      ...
      val painter = rememberImagePainter(data = characterDTO.image) {
        placeholder(R.drawable.splash_image)
        error(R.drawable.marvel_ic)
      }
      if (characterDTO.name.isNotEmpty()) {
        Image(
          painter = painter,
          contentDescription = "",
